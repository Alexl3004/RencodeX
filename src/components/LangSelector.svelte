<script lang="ts">
  ...
  let langs = $derived(
    mode === "audio" ? encoder.audioLangs : encoder.subLangs,
  );
  let selected = $derived(
    mode === "audio" ? encoder.selAudio : encoder.selSubs,
  );
    ...
    <div class="flex flex-wrap gap-1.5">
      {#each sortedLangs as lang}
        ...
        <button
          ...
          aria-label={isActive
            ? `Désactiver ${langName(lang)}`
            : `Activer ${langName(lang)}`}
          aria-pressed={isActive}
        >
          {#if isActive}
            <CircleCheck class="w-3 h-3 shrink-0" />
          {/if}
          <span class="text-[9px] tracking-wider" style="opacity: 0.7;"
